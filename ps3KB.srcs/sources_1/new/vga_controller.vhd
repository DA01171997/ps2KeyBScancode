...
ENTITY vga_controller IS
	GENERIC(
		...
		h_pixels	:	INTEGER := 639;		--horiztonal display width in pixels
		h_fp	 	:	INTEGER := 16;		--horiztonal front porch width in pixels
		h_pol		:	STD_LOGIC := '1';		--horizontal sync pulse polarity (1 = positive, 0 = negative)
		v_pulse 	:	INTEGER := 2;			--vertical sync pulse width in rows
		v_bp	 	:	INTEGER := 33;			--vertical back porch width in rows
		v_pixels	:	INTEGER := 479;		--vertical display width in rows
...
ARCHITECTURE behavior OF vga_controller IS
	CONSTANT	h_period	:	INTEGER := h_bp + h_pixels + h_fp + h_pulse;  --total number of pixel clocks in a row
	CONSTANT	v_period	:	INTEGER := v_bp + v_pixels + v_fp + v_pulse;  --total number of rows in column 



BEGIN
	        VGA_RED <= keyinput(7 downto 4);
VGA_GRN <= keyinput(3 downto 0);
VGA_BLU <= keyinput(3 downto 0);
	...
	BEGIN
     ...
     --report "v_count value :" &integer'image(v_count);
		...
		IF(rising_edge(pixel_clk)) THEN
            report "h_count value :" &integer'image(h_count);
			--counters
